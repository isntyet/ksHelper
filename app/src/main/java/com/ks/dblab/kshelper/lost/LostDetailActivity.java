...
public class LostDetailActivity extends BaseActivity {
    ...
    private RecyclerView rvLostCommentList;
    private List<CommentItem> lostCommentList;
    ...
    protected void createActivity(Bundle savedInstanceState) {
        View view = this.setContainerView(R.layout.activity_lost_detail);
        getSupportActionBar().setTitle("분실물 찾기/찾아주기");

        //Log.d("user_image", "asdf" + preference.getKakaoImage());
        ...
        rvLostCommentList = (RecyclerView) view.findViewById(R.id.rv_lost_comment_list);

        LinearLayoutManager layoutManager = new LinearLayoutManager(getApplicationContext());
        rvLostCommentList.setHasFixedSize(true);
        rvLostCommentList.setLayoutManager(layoutManager);

        lostCommentList = commentListRequest();

        //Log.d("size : " , lostCommentList.size() + "");
        resizeCommentList(lostCommentList.size());

        rvLostCommentList.setAdapter(new CommentAdapter(getApplicationContext(),lostCommentList, R.layout.activity_lost_detail));
        rvLostCommentList.addItemDecoration(new SimpleDividerItemDecoration(getApplicationContext()));
    ...
    private List<CommentItem> commentListRequest(){
        ...
        return request.startRequest("lost");
    }
    ...
    private String commentWriteRequest(String content){
        ...
        String responData = request.startRequest("lost");
    ...
    private void loadDialog() {
        ...
        dialog.setContentView(R.layout.item_dialog_lost_comment);
        ...
        dialog.findViewById(R.id.btn_save).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(etContent.length() == 0){
                    Toast.makeText(LostDetailActivity.this, "내용을 제대로 작성해 주세요", Toast.LENGTH_SHORT).show();
                } else {
                    //commentWriteRequest(etContent.getText().toString());
                    if("success".equals(commentWriteRequest(etContent.getText().toString()))){
                        commentCntRequest();
                        Toast.makeText(LostDetailActivity.this, "저장 완료", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(LostDetailActivity.this, "저장 실패", Toast.LENGTH_SHORT).show();
                    }
                    dialog.dismiss();
                    onResume();
    ...
    private void loadDeleteDialog() {
        dialog = new Dialog(this);
        dialog.setContentView(R.layout.item_dialog_lost_pwd);
        ...
        dialog.findViewById(R.id.btn_save).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(etPwd.length() != 6){
                    Toast.makeText(LostDetailActivity.this, "비밀번호를 입력해 주세요.", Toast.LENGTH_SHORT).show();
                } else if(!pwd.equals(etPwd.getText().toString())){
                    Toast.makeText(LostDetailActivity.this, "비밀번호가 틀렸습니다!", Toast.LENGTH_SHORT).show();
                } else{
                    String result = deleteRequest();
                    if("success".equals(result)){
                        Toast.makeText(LostDetailActivity.this, "삭제 완료", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(LostDetailActivity.this, "삭제 실패", Toast.LENGTH_SHORT).show();
                    }
                    dialog.dismiss();
                    finish();
                }
    ...
    protected void onResume() {
        lostCommentList = null;
        rvLostCommentList.removeAllViewsInLayout();
